...
// Below values are best output of multiple experiments 
...
// Play around with these coeff to get best driving expereince 
constexpr double coeff_cte = 1.;
constexpr double coeff_epsi = 1.;
constexpr double coeff_v = 1.;

constexpr double coeff_penalize_delta = 100.;  // Minimizes the use of steering. /Best = 100,200 // With value as 1 - the car dives into river near second turn
constexpr double coeff_penalize_a = 20.;  // Minimizes the use of acceleration. // With value as 100 - car drives smoothly and accurately but was steady and slow at 25 kmph

constexpr double coeff_derivative_delta = 10.; // Increases smoothness driving (smoothness steering) / Best = 10, even 100 works but 10 seems more smooth
constexpr double coeff_derivative_a = 100.;     // Increases smoothness of acceleration / Best = 10
              ...
              // The state at time t.
                ...
                // AD<double> psides0 = CppAD::atan(coeffs[1]);
                 ...
                 AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * pow(x0, 2) + coeffs[3] * pow(x0, 3);
                  AD<double> psides0 = CppAD::atan(coeffs[1]+ x0*coeffs[2]*2 + coeffs[3]*3*pow(x0,2));
...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs,vector<double> &path_x, vector<double> &path_y) {
  size_t i;
...
for (int i=0; i<N; i++){
    path_x.push_back(solution.x[i]);
    path_y.push_back(solution.x[i + N]); //N
  }
